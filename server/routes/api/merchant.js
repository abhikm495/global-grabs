...
router.post('/add', async (req, res) => {
  try {
    ...
    await brevoMail.sendEmail(email, 'merchant-application');
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await brevoMail.sendEmail(merchantDoc.email, 'merchant-deactivate-account');
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await brevoMail.sendEmail(email, 'merchant-welcome', null, name);
    ...
    await brevoMail.sendEmail(email, 'merchant-signup', host, {
      resetToken,
      email
    });
